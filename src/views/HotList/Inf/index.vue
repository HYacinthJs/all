<template>
    <div class="app-wrapper">
        <div style="position: fixed;z-index: 10;width: 100%;">
            <van-search v-model="input" shape="round" background="#2B8EFF" placeholder="请输入搜索关键词" disabled left>
                <template #left>
        ...
        <div v-for="(i, index) in infs">
            <el-card class="box-card">
                <div v-if="index != infs.length - 1" style="position: relative">
                    ...
                    <div style="display: inline-block;position: absolute;top: 10px;padding-left: 10px">{{ i.name }}
                    </div>
                </div>
                <div v-else>
...
export default {
    ...
    mounted() {
        ...
        this.$axios.get(`${this.$base}api/desc`, {
            params: {
