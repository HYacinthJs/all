    ...
    <section class="head-wrapper">
        <div class="hd-t">
            <div class="info">
                ...
                <span class="avatar bg-cover-all" v-else :style="{
                    backgroundImage: `${nickname ? 'url(' + user.avatar + ')'
                        : 'url(' + require('assets/images/account-circle.svg') + ')'}`
                }">
                </span>
                <span class="name" style="font-size: 20px;font-family: 微软雅黑;letter-spacing: 1px"
                    v-if="nickname">{{ this.$store.state.nickname }}
                    <span style="font-size: 15px;color: #ffffff;letter-spacing: 1px"
                        @click="$router.push('/homepage')">个人主页<i class="el-icon-arrow-right"></i></span>
                </span>
