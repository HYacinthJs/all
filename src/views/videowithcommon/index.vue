<template>
    <div class="article-container">
        <!-- 文章详情 -->
        <!-- 导航栏 -->
        <van-nav-bar class="page-nav-bar" left-arrow @click-left="onClickLeft" title="天天视频"></van-nav-bar>
                ...
                <div class="video-container">
                    <section class="item border-half-bottom" v-for="(item, index) in videoInfo">
                        <div class="video">
                            ...
                            <!-- canvas代替video播放 -->
                            <div class="canvas-video bg-cover"
                                :style="{ backgroundImage: `url(${item.tag + item.imgPath})` }">
                        ...
                        <comment-list :userId="nowUserId" :getMediaId="item.fileId" v-model="commentList"
                        ...
                        <div class="article-bottom">
                            ...
                            <!--&lt;!&ndash; 收藏图标  &ndash;&gt;-->
                            <colletion-video v-model="isCollectedVideo" class="btn-item" :video-id="item.fileId"
                                :userId="nowUserId"></colletion-video>

                        </div>
                        <!--发布评论-->
                        <van-popup v-model="isPostShow" position="bottom">
                            <!-- 把文章id传到发表评论的组件里 -->
                            <comment-post :target="item.fileId" @post-success="onPostSuccess"
...
export default {
    ...
    methods: {
        ...
        play(index, item) {
            ...
            axios.get(`${this.$base}video/list/playCount`, {
                params: {
                    id: item.fileId,
                }
            }
