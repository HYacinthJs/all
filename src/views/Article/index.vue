<template>
    <div class="article-container">
        <!-- 文章详情 -->
        <!-- 导航栏 -->
        <van-nav-bar class="page-nav-bar" left-arrow @click-left="onClickLeft" title="微笑头条"></van-nav-bar>
        ...
        <div class="main-wrap">
            ...
            <div class="article-detail" v-else-if="this.articleInfo.content">
                ...
                <van-cell class="user-info" center :border="false">
                    <van-image class="avatar" slot="icon" round fit="cover" :src="srcUrl" />
                    <div slot="title" class="user-name">{{ articleInfo.userName }}</div>
                    <div slot="label" class="publish-date">
                        {{ articleInfo.publishtime | relativeTime }}

                    </div>
                    <van-button :loading="followLoading" v-if="isFollowUser" class="follow-btn" round size="small"
                        @click="onFollow">已关注</van-button>
                    <van-button :loading="followLoading" class="follow-btn" v-else round type="info" color="#3296fa"
                        size="small" icon="plus" @click="onFollow">
                        关注
                    </van-button>
                </van-cell>
                ...
                <!--评论组件 -->
                <!--评论区域-->
                <comment-list :userId="nowUserId" :source="articleInfo.id" :list="commentList"
                ...
                <div class="article-bottom">
                    ...
                    <!--&lt;!&ndash; 收藏图标  &ndash;&gt;-->
                    <collection-article v-model="isCollectedArticle" class="btn-item" :article-id="articleInfo.id"
                        :userId="nowUserId"></collection-article>
                    <!--&lt;!&ndash; 点赞图标 &ndash;&gt;-->
                    <good-article v-model="articleAttitude" class="btn-item" :article-id="articleInfo.id"
                        :userId="nowUserId"></good-article>
                    <!--&lt;!&ndash; 分享图标 &ndash;&gt;-->
                </div>
                ...
                <van-popup v-model="isPostShow" position="bottom">
                    <!-- 把文章id传到发表评论的组件里 -->
                    <comment-post :artId="articleInfo.id" @post-message="updateReplyNum"
        ...
        <van-popup v-model="isReplyShow" position="bottom" style="height: 90%">
            <comment-reply v-if="isReplyShow" :comment="currentComment" :userId="nowUserId"
