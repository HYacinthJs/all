<template>
    <div>
        <HeaderBar title="视频评论"></HeaderBar>
        <div class="li border-half" @click="" v-for="mg in msg" v-if="mg.userid != uid">

            <div class="comuser">{{ mg.nickname }}<p>评论了你:</p>
            </div>
            <div class="content">{{ mg.content }}</div>
            <div class="works">{{ mg.videoId }}作品</div>
        </div>
...
export default {
    ...
    methods: {
        fn() {
            this.$axios.get(`${this.$base}msg/video/${this.uid}`).then((res) => {
